# E-Commerce Data Modeling & Customer Lifetime Value (CLV) Analytics
...
## Tech Stack
* **Ngôn ngữ & Thư viện:** Python (Pandas, NumPy, Scikit-learn, XGBoost, Imbalanced-learn)
* **Data Modeling:** Star Schema (Fact & Dimension Tables), T-SQL
* **Trực quan hóa (BI):** Power BI (DAX, Data Modeling)

## Tính năng & Workflow cốt lõi
...
## Điểm nhấn Kỹ thuật & Insight Kinh doanh (Business Insights)
...
## Cấu trúc Repository
* `data/raw/`: Dữ liệu thô nguyên bản (Kaggle).
* `data/star_schema/`: Dữ liệu đã được chuẩn hóa (Fact & Dim).
* `data/model/`: Kết quả đầu ra của mô hình ML (Customer Segments & Risk Scores).
* `notebooks/`: Chứa mã nguồn quá trình ETL, Phân cụm và Huấn luyện mô hình (EDA, SMOTE, XGBoost).
* `dashboard/`: File báo cáo Power BI (.pbix).
